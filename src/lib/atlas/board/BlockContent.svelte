<script lang="ts">
	...
	let { block, role, span }: { block: ResearchBoardBlock; role: ViewRole; span: number } = $props();
</script>

{#if block.kind === 'history'}
	<HistoryView {block} {span} />
...
{:else if block.kind === 'workspace_view'}
	...
	{:else if block.binding.view === 'metric_history'}<HistoryView {block} {span} />
	{:else if block.binding.view === 'peer_distribution'}<DistributionView {block} {span} />
	{:else if block.binding.view === 'change_attribution'}<AttributionView {block} {span} />
	{:else if block.binding.view === 'metric_relationship'}<RelationshipView {block} {span} />
	{:else if block.binding.view === 'headquarters_geography'}<GeographyView {block} {span} />
	{:else if block.binding.view === 'economic_context'}<EconomyView {block} {span} />
	{:else if block.binding.view === 'bank_context'}<RecordView {block} />
	{/if}
{:else if block.kind === 'analysis'}
	{#if block.binding.resultRef.kind === 'failure_pattern'}<FailurePatternView {block} {span} />
